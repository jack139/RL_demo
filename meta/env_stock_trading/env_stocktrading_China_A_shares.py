...
class StockTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        assert len(self.state)==self.state_space, "wrong state length!"


    def _sell_stock(self, index, action):
        def _do_sell_normal():
            if self.state[index + 1] > 0:
                #print(f"sell: {self.today} {self.state[0]} {index} {action} {self.state[index + self.stock_dim + 1]} {self.state[index + self.stock_dim*2 + 1]}")

                # Sell only if the price is > 0 (no missing data in this particular date)
                # perform sell action based on the sign of the action
                if self.state[index + self.stock_dim + 1] - self.state[index + self.stock_dim*2 + 1] > 0: # 减去今日禁售， t+1
                    # Sell only if current asset is > 0
                    sell_num_shares = min(
                        abs(action), self.state[index + self.stock_dim + 1] - self.state[index + self.stock_dim*2 + 1]  # 减去今日禁售， t+1
                    )
        ...
        #print(f"sold: {self.today} {self.state[0]} {index} {action} {self.state[index + self.stock_dim + 1]} {self.state[index + self.stock_dim*2 + 1]}")
    ...
    def _buy_stock(self, index, action):
        def _do_buy():
            if self.state[index + 1] > 0:
                ...
                #print(f"buy: {self.today} {self.state[0]} {index} {action} {self.state[index + self.stock_dim + 1]} {self.state[index + self.stock_dim*2 + 1]}")
                ...
                self.state[index + self.stock_dim*2 + 1] += buy_num_shares # 当日的禁售， t+1
        ...
        #print(f"bought: {self.today} {self.state[0]} {index} {action} {self.state[index + self.stock_dim + 1]} {self.state[index + self.stock_dim*2 + 1]}")
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            if self.initial_stocks: # 有 初始库存
                begin_total_asset += sum(
                    np.array(self.state[1 : (self.stock_dim + 1)])
                    * np.array(self.initial_stocks[0])
                )
            ...
            return self.state, self.reward, self.terminal, {"cost":self.cost}
            ...
            today = self._get_date()
            self.date_memory.append(today)
            if today.split()[0] != self.today: # 日期不同了，可以卖了
                self.today = today.split()[0]
                #print("before:", self.state)
                self.state[(self.stock_dim*2 + 1) : (self.stock_dim*3 + 1)] = [0] * self.stock_dim # 禁售库存清零
    ...
    def _initiate_state(self):
        if self.initial:
            if self.initial_stocks: # 有 初始库存
                initial_stocks = self.initial_stocks[0] # 库存
                initial_no_sell = self.initial_stocks[1] # 当日禁售

                assert len(initial_stocks)==self.stock_dim, "wrong initial_stocks length"
                assert len(initial_no_sell)==self.stock_dim, "wrong initial_no_sell length"
            else:
                initial_stocks = [0] * self.stock_dim # 库存
                initial_no_sell = [0] * self.stock_dim # 当日禁售
            ...
            if len(self.df.tic.unique()) > 1:
                ...
                state = (
                    ...
                    + self.data.close.values.tolist()
                    + self.previous_state[
                        (self.stock_dim + 1) : (self.stock_dim * 3 + 1)
                    ]
                    + sum(
                        [
                            self.data[tech].values.tolist()
        ...
        assert len(state)==self.state_space, "wrong state length!"
    ...
    def _update_state(self):
        if len(self.df.tic.unique()) > 1:
            ...
            state = (
                [self.state[0]]
                + self.data.close.values.tolist()
                + list(self.state[(self.stock_dim + 1) : (self.stock_dim * 3 + 1)])
                + sum(
                    [
                        self.data[tech].values.tolist()
                        for tech in self.tech_indicator_list
